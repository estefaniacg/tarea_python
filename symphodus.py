...
map.drawmapboundary()
map.drawmeridians(np.arange(0, 360, 30), labels=[False, False, False, True], linewidth=0.2)
map.drawparallels(np.arange(-90, 90, 30), labels=[False, True, False, False], linewidth=0.2)
...
specie =pd. read_csv('symphodus.csv' , sep=',',
...
map.plot(lon, lat, 'bo', markersize=1.3, markeredgecolor='none')

# INSTRUCCION
# Debeis guardar la figura a un archivo pdf
plt.savefig("symphodus.pdf")
